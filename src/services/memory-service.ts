...
import type { AgentScope, AuditTrailEntry, MutationSummary, WorkspaceContext } from '../db/repository-types.js';
...
export class MemoryService {
   ...
   */
  async storeVerbatim(userId: string, content: string, sourceSite: string, sourceUrl: string = '', effectiveConfig?: MemoryServiceDeps['config']): Promise<IngestResult> {
    return performStoreVerbatim(this.depsFor(effectiveConfig), userId, content, sourceSite, sourceUrl);
  }
   ...
   * caller — typically the BEAM harness — knows the mapping). Returns the
   * parsed events. Best-effort: if the underlying LLM call fails or the
   * service is not wired, returns `[]` without throwing.
   */
